...
def calculate_rmsd(rf_pdb, af_pdb, binder_chain_rf='B', binder_chain_af='A'):
    ...
    try:
        ...
        rf_binder = rf_structure[0][binder_chain_rf]
        af_binder = af_structure[0][binder_chain_af]
        
        rf_atoms = []
        af_atoms = []
        
        rf_residues = list(rf_binder.get_residues())
        af_residues = list(af_binder.get_residues())
        
        min_length = min(len(rf_residues), len(af_residues))
        
        for i in range(min_length):
            try:
                rf_atoms.append(rf_residues[i]["CA"])
                af_atoms.append(af_residues[i]["CA"])
            except KeyError:
                continue
        
        super_imposer = Superimposer()
        super_imposer.set_atoms(rf_atoms, af_atoms)
        
        return super_imposer.rms
    
    except Exception as e:
        print(f"Error calculating RMSD between {rf_pdb} and {af_pdb}: {e}")
        return None
...
def analyze_design(af_folder, rf_folder, output_csv,
                   binder_chain_af = "A", binder_chain_rf = "B",
                   target_chain_af = "B", target_chain_rf = "D"):
    ...
    for design_dir in tqdm(design_folders, desc="Analyzing designs"):
        ...
        # Loop through each AF2 model rank
        for i, (model_file, score_file) in enumerate(zip(sorted(model_files), sorted(score_files))):
            ...
            # Get chain lengths
            binder_length = get_chain_length(model_file, chain_id=binder_chain_af)
            target_length = get_chain_length(model_file, chain_id=target_chain_af)
            ...
            try:
                ...
                rmsd = calculate_rmsd(rf_pdb = rf_backbone, af_pdb = model_file,
                                      binder_chain_rf=binder_chain_rf,
                                      binder_chain_af=binder_chain_af)
...
def main():
    ...
    parser.add_argument('--binder_chain_rf', default = "B", help='Chain ID of the binder from RFdiffusion')
    parser.add_argument('--binder_chain_af', default = "A", help='Chain ID of the binder from AlphaFold')
    ...
    df = analyze_design(args.af_folder, args.rf_folder, default_csv,
                        binder_chain_af = args.binder_chain_af, binder_chain_rf = args.binder_chain_rf)
